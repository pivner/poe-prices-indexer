...
def main():
    ...
    flipvalue = [((c["lines"][i]["chaosEquivalent"]-c["lines"][i]["receive"]["value"])) for i in range(len(c["lines"]))] if r[1] else [((c["lines"][i]["chaosValue"])*(c["lines"][i]["sparkline"]["totalChange"]/100)) for i in range(len(c["lines"]))]
