...
def main():
    ...
    if r[1]:
        flipvalue = [((c["lines"][i]["chaosEquivalent"]-c["lines"][i]["receive"]["value"])) for i in range(len(c["lines"]))]
    else:
        flipvalue = [(c["lines"][i]["chaosValue"])+(c["lines"][i]["chaosValue"])*(c["lines"][i]["sparkline"]["totalChange"]/100) for i in range(len(c["lines"]))]
